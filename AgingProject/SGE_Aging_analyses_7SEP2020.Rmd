...
cellcount.mat2 <- as.data.frame(matrix(nrow = 14, ncol = 7))
#add cluster together to get cell type-level counts
cellcount.mat2[,1] <- cellcount.mat[,1] + cellcount.mat[,4]
cellcount.mat2[,2] <- cellcount.mat[,2]
cellcount.mat2[,3] <- cellcount.mat[,3]
cellcount.mat2[,4] <- cellcount.mat[,5] + cellcount.mat[,9]
cellcount.mat2[,5] <- cellcount.mat[,6]
cellcount.mat2[,6] <- cellcount.mat[,7]
cellcount.mat2[,7] <- cellcount.mat[,8]
...
flow_props$V3 <- flow_props.tmp$CD8.cells
flow_props$V4 <- flow_props.tmp$B.cells..CD8.. + flow_props.tmp$B.cells..CD8...1
flow_props$V5 <- flow_props.tmp$NK.cells
flow_props$V6 <- flow_props.tmp$Class..Mono..CD14.CD16..
flow_props$V7 <- flow_props.tmp$Act..Mono..CD14.CD16.. + flow_props.tmp$Act..Mono..CD14.CD16...1
...
Correlations between single cell and flow proportions vary across cell types. To summary, moderately good positive correlation for CD4 T cells, neither of the CD8 props look very good. NK cells, B cells, and both monocytes populations look very strongly correlated.
...
qplot(age, pca$x[,4], col=age,size=1) + xlab('age') + ylab(paste("PC4","\nPVE =",round(summary(pca)$imp[2,4],2))) + stat_smooth(method = 'lm', color='black', size = 1) 
...
In this updated analysis age appears to be most strongly correlated with PC4, which explains ~11% of the variance.
...
Now I'll run a linear model to test the association between age and PC1 of gene expression while controlling for the top 2 PCs of cell composition
```{r}
knitr::kable(summary(lm(pca_AS$x[,1] ~ age + pca$x[,1] + pca$x[,2]))$coefficients, format = 'simple', digits = 3)
```
...
p_all
```
```{r CD8_A+ T cells, warning=FALSE, message=FALSE}
...
p_all
```
```{r CD8_B+ T cells, warning=FALSE, message=FALSE}
